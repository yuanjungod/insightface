    ...
    if not binarize:
        if not dcn:
            ...
            conv1 = mx.contrib.symbol.DeformableConvolution(name=name + "_conv", data=act1, offset=conv1_offset,
                                                            ...
                                                            num_deformable_group=1, stride=(1, 1), dilate=(1, 1),
                                                            no_bias=False)
        ...
        conv1 = mx.sym.QConvolution_v1(data=act1, num_filter=num_filter, kernel=(1, 1), stride=(1, 1), pad=(0, 0),
                                       no_bias=True, workspace=workspace, name=name + '_conv', act_bit=ACT_BIT,
                                       weight_bit=bit)
...
def lin3(data, num_filter, workspace, name, k, g=1, d=1):
    ...
    # return conv1
    if k != 3:
        conv1 = Conv(data=data, num_filter=num_filter, kernel=(k, k), stride=(1, 1), pad=((k - 1) // 2, (k - 1) // 2),
                     num_group=g,
                     no_bias=True, workspace=workspace, name=name + '_conv')
    else:
        conv1 = Conv(data=data, num_filter=num_filter, kernel=(k, k), stride=(1, 1), pad=(d, d), num_group=g,
                     dilate=(d, d),
                     no_bias=True, workspace=workspace, name=name + '_conv')
    bn1 = mx.sym.BatchNorm(data=conv1, fix_gamma=False, momentum=bn_mom, eps=2e-5, name=name + '_bn')
    act1 = Act(data=bn1, act_type='relu', name=name + '_relu')
...
def residual_unit_a(data, num_filter, stride, dim_match, name, binarize, dcn, dilate, **kwargs):
    ...
    if not binarize:
        ...
        conv1 = mx.sym.QConvolution(data=act1, num_filter=int(num_filter * 0.5), kernel=(1, 1), stride=(1, 1),
                                    pad=(0, 0),
                                    no_bias=True, workspace=workspace, name=name + '_conv1', act_bit=ACT_BIT,
                                    weight_bit=bit)
    ...
    if not binarize:
        ...
        conv2 = mx.sym.QConvolution(data=act2, num_filter=int(num_filter * 0.5), kernel=(3, 3), stride=(1, 1),
                                    pad=(1, 1),
                                    no_bias=True, workspace=workspace, name=name + '_conv2', act_bit=ACT_BIT,
                                    weight_bit=bit)
    ...
    if not binarize:
        ...
        conv3 = mx.sym.QConvolution(data=act3, num_filter=num_filter, kernel=(1, 1), stride=(1, 1), pad=(0, 0),
                                    no_bias=True, workspace=workspace, name=name + '_conv3', act_bit=ACT_BIT,
                                    weight_bit=bit)
    # if binarize:
    ...
    if dim_match:
        ...
        if not binarize:
            ...
            shortcut = mx.sym.QConvolution(data=act1, num_filter=num_filter, kernel=(1, 1), stride=stride, pad=(0, 0),
                                           no_bias=True, workspace=workspace, name=name + '_sc', act_bit=ACT_BIT,
                                           weight_bit=bit)
    if memonger:
...
def residual_unit_g(data, num_filter, stride, dim_match, name, binarize, dcn, dilation, **kwargs):
    ...
    if not binarize:
        act1 = Act(data=bn1, act_type='relu', name=name + '_relu1')
        if not dcn:
            conv1 = Conv(data=act1, num_filter=int(num_filter * 0.5), kernel=(3, 3), stride=(1, 1),
            ...
            conv1 = mx.contrib.symbol.DeformableConvolution(name=name + '_conv1', data=act1, offset=conv1_offset,
                                                            num_filter=int(num_filter * 0.5), pad=(1, 1), kernel=(3, 3),
                                                            num_deformable_group=1, stride=(1, 1), dilate=(1, 1),
                                                            no_bias=True)
    else:
        act1 = mx.sym.QActivation(data=bn1, act_bit=ACT_BIT, name=name + '_relu1', backward_only=True)
        conv1 = mx.sym.QConvolution_v1(data=act1, num_filter=int(num_filter * 0.5), kernel=(3, 3), stride=(1, 1),
                                       pad=(1, 1),
                                       no_bias=True, workspace=workspace, name=name + '_conv1', act_bit=ACT_BIT,
                                       weight_bit=bit)
    bn2 = mx.sym.BatchNorm(data=conv1, fix_gamma=False, eps=2e-5, momentum=bn_mom, name=name + '_bn2')
    if not binarize:
        act2 = Act(data=bn2, act_type='relu', name=name + '_relu2')
        if not dcn:
            conv2 = Conv(data=act2, num_filter=int(num_filter * 0.25), kernel=(3, 3), stride=(1, 1),
                         pad=(dilation, dilation), dilate=(dilation, dilation),
                         no_bias=True, workspace=workspace, name=name + '_conv2')
        else:
            ...
            conv2 = mx.contrib.symbol.DeformableConvolution(name=name + '_conv2', data=act2, offset=conv2_offset,
                                                            num_filter=int(num_filter * 0.25), pad=(1, 1),
                                                            kernel=(3, 3), num_deformable_group=1, stride=(1, 1),
                                                            dilate=(1, 1), no_bias=True)
    else:
        act2 = mx.sym.QActivation(data=bn2, act_bit=ACT_BIT, name=name + '_relu2', backward_only=True)
        conv2 = mx.sym.QConvolution_v1(data=act2, num_filter=int(num_filter * 0.25), kernel=(3, 3), stride=(1, 1),
                                       pad=(1, 1),
                                       no_bias=True, workspace=workspace, name=name + '_conv2', act_bit=ACT_BIT,
                                       weight_bit=bit)
    bn3 = mx.sym.BatchNorm(data=conv2, fix_gamma=False, eps=2e-5, momentum=bn_mom, name=name + '_bn3')
    if not binarize:
        act3 = Act(data=bn3, act_type='relu', name=name + '_relu3')
        if not dcn:
            conv3 = Conv(data=act3, num_filter=int(num_filter * 0.25), kernel=(3, 3), stride=(1, 1),
                         pad=(dilation, dilation), dilate=(dilation, dilation),
        ...
        act3 = mx.sym.QActivation(data=bn3, act_bit=ACT_BIT, name=name + '_relu3', backward_only=True)
        conv3 = mx.sym.QConvolution_v1(data=act3, num_filter=int(num_filter * 0.25), kernel=(3, 3), stride=(1, 1),
                                       pad=(1, 1),
                                       no_bias=True, workspace=workspace, name=name + '_conv3', act_bit=ACT_BIT,
                                       weight_bit=bit)
    ...
    if dim_match:
        ...
        if not binarize:
            ...
            # assert(False)
            shortcut = mx.sym.QConvolution_v1(data=act1, num_filter=num_filter, kernel=(1, 1), stride=stride,
                                              pad=(0, 0),
                                              no_bias=True, workspace=workspace, name=name + '_sc', act_bit=ACT_BIT,
                                              weight_bit=bit)
...
class DLA:
    ...
    def get_output(self, w, h):
        ...
        if h == self.n:
            ...
            if use_DLA < 10:
                if h % 2 == 1 and h != w:
                    xbody = lin3(x[0], self.nFilters, self.workspace, "%s_w%d_h%d_x" % (self.name, w, h), 3,
                                 self.nFilters, 1)
                    HC = True
                    # xbody = x[0]
                else:
            ...
            if x[1] // y[1] == 2:
                if w > 1:
                    ...
                    ybody = mx.sym.BatchNorm(data=ybody, fix_gamma=False, momentum=bn_mom, eps=2e-5,
                                             name="%s_w%d_h%d_y_bn" % (self.name, w, h))
                    ...
                    if h >= 1:
                        ybody = mx.symbol.Deconvolution(data=y[0], num_filter=self.nFilters, kernel=(s, s),
                                                        stride=(s, s),
                                                        num_group=self.nFilters, no_bias=True,
                                                        name='%s_upsampling_w%d_h%d' % (self.name, w, h),
                                                        attr={'lr_mult': '0.0', 'wd_mult': '0.0'},
                        ...
                        ybody = Act(data=ybody, act_type='relu', name="%s_w%d_h%d_y_act" % (self.name, w, h))
                        ybody = Conv(data=ybody, num_filter=self.nFilters, kernel=(3, 3), stride=(1, 1), pad=(1, 1),
                                     no_bias=True, name="%s_w%d_h%d_y_conv2" % (self.name, w, h),
                                     workspace=self.workspace)
                        ybody = mx.sym.BatchNorm(data=ybody, fix_gamma=False, momentum=bn_mom, eps=2e-5,
                                                 name="%s_w%d_h%d_y_bn2" % (self.name, w, h))
                        ybody = Act(data=ybody, act_type='relu', name="%s_w%d_h%d_y_act2" % (self.name, w, h))
            else:
            ...
            if use_DLA < 10:
                if use_DLA > 1 and h == 3 and w == 2:
                    ...
                    zbody = mx.sym.Pooling(data=zbody, kernel=(z[1], z[1]), stride=(z[1], z[1]), pad=(0, 0),
                                           pool_type='avg')
...
def get_symbol(num_classes, **kwargs):
    ...
    if use_STN:
        ...
        loc_net = mx.sym.BatchNorm(data=loc_net, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='stn_bn0',
                                   attr={'lr_mult': lr_mult})
        ...
        loc_net = mx.sym.BatchNorm(data=loc_net, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='stn_bn1',
                                   attr={'lr_mult': lr_mult})
        ...
        loc_net = mx.sym.FullyConnected(data=loc_net, num_hidden=int(nFilters * 0.5), name='loc_net_half',
                                        attr={'lr_mult': lr_mult})
        ...
        data = mx.sym.SpatialTransformer(data=data, loc=loc_theta, target_shape=(input_size, input_size),
                                         transform_type="affine", sampler_type="bilinear")
        ref_label = mx.sym.SpatialTransformer(data=ref_label, loc=loc_theta, target_shape=(label_size, label_size),
                                              transform_type="affine", sampler_type="bilinear")
    ...
    body = residual_unit(body, nFilters, (1, 1), mFilters == nFilters, 'res2', binarize, dcn, 1,
    ...
    for i in xrange(nStacks):
        ...
        if not _dcn:
            ...
            out = mx.contrib.symbol.DeformableConvolution(name=_name, data=ll, offset=out_offset,
                                                          num_filter=num_classes, pad=(1, 1), kernel=(3, 3),
                                                          num_deformable_group=1, stride=(1, 1), dilate=(1, 1),
                                                          no_bias=False)
        ...
        if use_coherent > 0:
            ...
            if use_coherent == 1:
                ...
                ux = mx.sym.SpatialTransformer(data=ux, loc=m, target_shape=(label_size, label_size),
                                               transform_type='affine', sampler_type='bilinear')
                closs = l2_loss(ux, dx)
                closs = closs / nStacks
                closses.append(closs)
        ...
        if i < nStacks - 1:
            ...
            if _dcn:
                ...
                out3 = mx.contrib.symbol.DeformableConvolution(name=_name, data=body, offset=out3_offset,
                                                               num_filter=nFilters, pad=(1, 1), kernel=(3, 3),
                                                               num_deformable_group=1, stride=(1, 1), dilate=(1, 1),
                                                               no_bias=False)
                body = out3
            # elif use_STN:
